...
singletons <- which(rowSums(mat)!=0)
mat <- mat[singletons,singletons]
...
network <- graph.adjacency(mat, mode="undirected", weighted=TRUE)
layout <- layout.drl(network,options=list(simmer.attraction=0))
...
# labels <- c("Dartmouth", "Home", "Phi Tau", "PBS/Uppver Valley", "Veronica's friends", "Others")
d$label <- NA
# d$label[unlist(central.nodes)] <- labels
# d$label <- unlist(fc$names)
